...
export function genChar(statPoints, speciesId, sizeRange, bodyTypeRange, genderId, name, pronounKey) {
    ...
    var credits = Math.floor(character.measurements.height * randomPercent);
...
function getExpCalc(character, exp, expMod, statPoints) {
    ...
    switch (exp) {
        case 'muscle':
            return (Math.round(Math.pow(character.stats.strg, 0.7)) * expMod) * hyperMod
        case 'fat':
            return (Math.round(Math.log10(character.stats.fat)) * expMod) * hyperMod
        case 'size':
            return (Math.round(Math.pow(character.measurements.height, 1.5)) * expMod) * hyperMod
        case 'skill':
            return (Math.floor(Math.log2(statPoints))) * hyperMod
        case 'pawEye':
            return (Math.round(Math.pow(character.stats.acc, 0.7)) * expMod) * hyperMod
        case 'agility':
            return (Math.round(character.stats.dex) * expMod) * hyperMod
        case 'physique':
            return (Math.round(Math.pow(character.stats.con, 0.7)) * expMod) * hyperMod
    }
}
